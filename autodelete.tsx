...
import { UserStore, Toasts, SelectedChannelStore, FluxDispatcher } from "@webpack/common";
...
function parseTime(input: string): number | null {
    ...
    switch (unit) {
        ...
        case 'm':
        case 'min':
            return value * 60; // minutos -> segundos
        default:
            return value;
    }
}
...
function processCommand(content: string) {
    ...
    if (command === "autodelete" || command === "ad") {
        ...
        if (args.length === 0) {
            Toasts.show({
                message: `📌 AutoDelete: ${enabled ? "✅ Ativo" : "❌ Inativo"} - ${formatTime(delay)}`,
                id: Toasts.genId(),
        ...
        if (arg === "status") {
            Toasts.show({
                message: `📌 ${enabled ? "✅ Ativo" : "❌ Inativo"} - ${formatTime(delay)} | ${activeTimers.size} mensagens agendadas`,
                id: Toasts.genId(),
                type: Toasts.Type.INFO
            });
            return true;
        ...
        // Verifica se é um tempo (ex: 10s, 5s, 1m, 2m, 30s)
        const seconds = parseTime(arg);
        if (seconds !== null && seconds >= 1 && seconds <= 3600) { // Máximo 1 hora
            delay = seconds;
            Toasts.show({
            ...
            Toasts.show({
                message: "❌ Delay deve ser entre 1s e 60m (1 hora)",
                id: Toasts.genId(),
                type: Toasts.Type.FAILURE
            });
            return true;
        ...
        Toasts.show({
            message: "📌 Comandos: autodelete [on/off/status/10s/1m/5m]",
            id: Toasts.genId(),
            type: Toasts.Type.INFO
        });
        return true;
...
export default definePlugin({
    ...
    start() {
        (window as any).autoDelete = {
            ...
            status: () => {
                Toasts.show({
                    message: `📌 ${enabled ? "✅ Ativo" : "❌ Inativo"} - ${formatTime(delay)} | ${activeTimers.size} agendadas`,
                    id: Toasts.genId(),
                    type: Toasts.Type.INFO
                });
            },
        ...
        logger.info("📌 Comandos: autodelete [on/off/status/10s/1m/5m]");
